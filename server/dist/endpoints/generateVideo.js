...
router.post('/', async (req, res) => {
    const { script, voiceValue, themeValue, musicValue, audienceValue, userEmail, } = req.body;
    ...
    if (musicValue == 'emotional') {
        ...
        if (musicExists == 0) {
            //converting to buffer
            const musicFileAbsolutePath = path.resolve(__dirname, '../musicTracks/emotional_333684.mp3');
            //reading content of file to a buffer
            const bufferMusic = await fs.readFile(musicFileAbsolutePath);
            //converting buffer to string
            const convertedBufferMusic = await convertBufferRedis(bufferMusic);
    ...
    if (musicValue == 'epic') {
        ...
        if (musicExists == 0) {
            //converting to buffer
            const musicFileAbsolutePath = path.resolve(__dirname, '../musicTracks/epic_364885.mp3');
            //reading content of file to a buffer
            const bufferMusic = await fs.readFile(musicFileAbsolutePath);
            //converting buffer to string
            const convertedBufferMusic = await convertBufferRedis(bufferMusic);
    ...
    if (musicValue == 'uplifting') {
        ...
        if (musicExists == 0) {
            //converting to buffer
            const musicFileAbsolutePath = path.resolve(__dirname, '../musicTracks/uplifting_328600.mp3');
            //reading content of file to a buffer
            const bufferMusic = await fs.readFile(musicFileAbsolutePath);
            //converting buffer to string
            const convertedBufferMusic = await convertBufferRedis(bufferMusic);
