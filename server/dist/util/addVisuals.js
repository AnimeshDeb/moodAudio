...
*/
export async function AddVisuals(userEmail, theme) {
    // Redis setup
    ...
    // Get combined audio
    const combinedAudioString = await redis.get(`${userEmail}:combined`);
    if (!combinedAudioString)
        return null;
    const combinedAudioBuffer = Buffer.from(combinedAudioString, 'base64');
    // Get images
    const imageArr = await getImages(theme);
    if (!imageArr.length)
        throw new Error('No images found for the theme.');
    // FFmpeg binary
    if (!ffmpeg)
            ...
            while (true) {
                for (const url of imageArr) {
                    ...
                    if (!imagePipe.write(buf)) {
                        await new Promise((resolve) => imagePipe.once('drain', resolve));
                    }
                }
            }
        }
