...
dotenv.config();
export async function voiceoverAndMusic(userEmail, musicValue) {
    const __filename = fileURLToPath(import.meta.url);
    const __dirname = path.dirname(__filename);
    const musicFileAbsolutePath = path.resolve(__dirname, '../musicTracks/ambient_345093.mp3');
    ...
    const base64 = await redis.get(userEmail);
    if (!base64)
        return null;
    //fetching music track from redis
    const music64 = await redis.get(`music: ${musicValue}`);
    if (!music64)
        return null;
    ...
    return new Promise((resolve, reject) => {
        ...
        const ffmpeg = spawn(ffmpegPath, [
            ...
            '-filter_complex',
            '[1:a]volume=0.065[a1];[0:a][a1]amix=inputs=2:duration=first:dropout_transition=2',
            '-f',
            'mp3', // output format
        ...
        const outputChunks = [];
        if (!ffmpeg.stdout)
            return reject(new Error('ffmpeg stdout is null'));
        ...
        ffmpeg.on('close', async (code) => {
            if (code === 0) {
                ...
                const combinedBase64 = await redis.get(`${userEmail}:combined`); //getting combined audio string from redis
                if (!combinedBase64)
                    return null;
        ...
        const musicStdin = ffmpeg.stdio[3];
